...
**Response**: It is un-tidy because each variable does not have its own
column. We should have a variable for season and a variable for word
count.
...
``` r
by_speaker %>% 
summarise(sum(contains(Season)))
```

    ## Error: Problem with `summarise()` input `..1`.
    ## ℹ `..1 = sum(contains(Season))`.
...
``` r
speaker_wide <- speaker_tidy %>% 
  pivot_wider(names_from = Speaker, values_from = Words)
knitr::kable(speaker_wide)
```
...
``` r
(us_wide <- pivot_wider(us_rent_income, values_from=c(estimate, moe), 
...
``` r
us_long <- us_wide %>% 
  pivot_longer(cols = income_estimate:rent_moe, names_to = c('variable', 'estimate','moe'), values_to = 'count')
```
